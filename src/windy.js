...
export var Windy = function (params) {
  ...
  var animate = function (bounds, field) {
    ...
    // console.log(params.canvas.getContext("webgl"));
  ...
  var stop = function () {
    ...
    if (params.canvas) {
      params.canvas.style.display = "none";
    }
  };
...
// create a custom style layer
export const createWindyLayer = (layerId, canvasId, options) => {
  return {
    id: layerId,
    type: "custom",
    options: { ...options },
    ...
    onAdd: function (map, gl) {
      this._map = map;
      this._layerCanvas = document.getElementById(canvasId);
    },
    onRemove: function () {
      this._destroyWind();
    },
    ...
    _getMapContainerSize: function () {
      var container = this._map.getContainer();
      return { x: container.clientWidth, y: container.clientHeight };
    },

    _startWindy: function () {
      var bounds = this._map.getBounds().toArray();

      const mapSize = this._getMapContainerSize();

      this._layerCanvas.width = mapSize.x;
      this._layerCanvas.height = mapSize.y;
    ...
    _initWindy: function (self) {
      ...
      const options = Object.assign(
        { canvas: this._layerCanvas, map: this._map },
        self.options
      );
      ...
      this._context = this._layerCanvas.getContext("2d");

      this._map.triggerRepaint();

      // stop wind on drag or zoom and rerender.
      // Wind is rerendered automatically on end by the render method of the layer
      this._map.on("dragstart", self._stopWind.bind(this));
      this._map.on("zoomstart", self._stopWind.bind(this));
      this._map.on("resize", self._stopWind.bind(this));
    },
    _stopWind: function () {
